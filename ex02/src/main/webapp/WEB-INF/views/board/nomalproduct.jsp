...
<body>
  ...
  <a class="carousel-control-next" href="#carouselExampleIndicators" role="button" data-slide="next">
    ...
    <span class="sr-only">Next</span>
				...
				<c:forEach items="${nomalproductlist}" var="board" begin="1" end="1">	
					<c:choose>
						<c:when test="${board.content eq '운동화_스니커즈'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">스니커즈</h1>
						</c:when>
						<c:when test="${board.content eq '운동화_캔버스화'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">캔버스화</h1>
						</c:when>
						<c:when test="${board.content eq '운동화_슬립온'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">슬립온</h1>
						</c:when>
						<c:when test="${board.content eq '스포츠_런닝화'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">런닝화</h1>
						</c:when>
						<c:when test="${board.content eq '스포츠_골프화'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">골프화</h1>
						</c:when>
						<c:when test="${board.content eq '스포츠_등산화'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">등산화</h1>
						</c:when>
						<c:when test="${board.content eq '구두_로퍼'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">로퍼</h1>
						</c:when>
						<c:when test="${board.content eq '구두_힐'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">힐</h1>
						</c:when>
						<c:when test="${board.content eq '샌들_성인'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">샌들</h1>
						</c:when>
						<c:when test="${board.content eq '키즈_샌들'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">키즈 샌들</h1>
						</c:when>
						<c:when test="${board.content eq '키즈_운동화'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">키즈 운동화</h1>
						</c:when>
						<c:when test="${board.content eq '키즈_스포츠'}">
							<h1 style ="margin-top:30px; margin-bottom:30px;" id ="pc_se1_text">키즈 스포츠</h1>
						</c:when>
					</c:choose>						
				</c:forEach>					
		...
		<c:choose>
				<c:when test="${!empty nomalproductlist}">
					<c:set var="cnt" value="0"/>
					<c:forEach items="${nomalproductlist}" var="board">	
  			...
  			<ul class="pagination pagination-lg justify-content-center">
				<c:if test="${pageMaker.prev }">
					<li class ="page-item disabled"><a class="page-link"
						href='<c:url value="/board/nomalproduct?page=${pageMaker.startPage-1 }"/>'><i
							class="fa fa-chevron-left"></i></a></li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage }"
					end="${pageMaker.endPage }" var="pageNum">
					<li class ="page-item"><a class="page-link"
						href='<c:url value="/board/nomalproduct?pageNum=${pageNum }"/>'><i
							class="fa">${pageNum }</i></a></li>
				</c:forEach>
				<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
					<li class ="page-item"><a class="page-link"
						href='<c:url value="/board/nomalproduct?page=${pageMaker.endPage+1 }"/>'><i
		...
		<div class="autoplay" id="mobileslider">
			<c:choose>
				<c:when test="${!empty nomalproductlist}">
					<c:forEach items="${nomalproductlist}" var="recent">	
								<div>
